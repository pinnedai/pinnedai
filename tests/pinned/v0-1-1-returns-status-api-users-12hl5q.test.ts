...
// ═══════════════════════════════════════════════════════════════
// Original PR claim: "/api/users returns 422 on invalid email"
// Source PR:         v0-1-1
...
// minimally-invalid body ("invalid email") and
// asserts the response status code is 422.
//
// Retire when no longer applicable:
//   pinned retire v0-1-1-returns-status-api-users-12hl5q --reason="..."
// ═══════════════════════════════════════════════════════════════
...
const CONDITION = "invalid email";
const ORIGINAL_PR = "v0-1-1";
const ORIGINAL_CLAIM = "/api/users returns 422 on invalid email";
const TEST_FILENAME = "v0-1-1-returns-status-api-users-12hl5q.test.ts";
...
describe("pinned: returns-status " + METHOD + " " + ROUTE + " → " + EXPECTED_STATUS, () => {
  ...
  it("returns " + EXPECTED_STATUS + " on " + CONDITION, async () => {
    ...
    const body = JSON.stringify({ "email": "INVALID_FOR_PINNED_TEST" });
