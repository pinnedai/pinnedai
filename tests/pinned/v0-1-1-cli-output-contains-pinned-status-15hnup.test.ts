...
// ═══════════════════════════════════════════════════════════════
// Original PR claim: "`pinned status` outputs `Pinned status`"
// Source PR:         v0-1-1
// Template:          cli-output-contains
// Permanent:         this test fails if the claim is ever regressed.
//
// Retire when no longer applicable:
//   pinned retire v0-1-1-cli-output-contains-pinned-status-15hnup --reason="..."
// ═══════════════════════════════════════════════════════════════

import { describe, it, expect } from "vitest";
import { execFileSync } from "node:child_process";

const COMMAND = "pinned status";
const ARGV = ["pinned","status"];
const EXPECTED_TEXT = "Pinned status";
const ORIGINAL_PR = "v0-1-1";
const ORIGINAL_CLAIM = "`pinned status` outputs `Pinned status`";
...
function repairPrompt(actualOutput: string): string {
  ...
  return [
    ...
    "",
    "After fixing, re-run:  npx vitest run tests/pinned/v0-1-1-cli-output-contains-pinned-status-15hnup.test.ts",
    "═══════════════════════════════════════════════════════════════",
    "",
  ].join("\n");
}
