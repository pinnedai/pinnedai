...
// Original PR claim: "`node ./apps/cli/dist/cli.js check --description \"Rate-limits /api/users to 60 req/min.\"` outputs `Found 1 claim(s)`"
// Pinned to fail if this is ever regressed.
//
// Retire when no longer applicable:
//   pinned retire v0-1-1-cli-output-contains-node-apps-cli-dist-cli-js-check-description-rate-limits-api-users-to-60-req-min-7llvs2 --reason="..."

import { describe, it, expect } from "vitest";
import { execFileSync } from "node:child_process";

const COMMAND = "node ./apps/cli/dist/cli.js check --description \"Rate-limits /api/users to 60 req/min.\"";
const ARGV = ["node","./apps/cli/dist/cli.js","check","--description","Rate-limits /api/users to 60 req/min."];
const EXPECTED_TEXT = "Found 1 claim(s)";
const ORIGINAL_PR = "v0-1-1";
const ORIGINAL_CLAIM = "`node ./apps/cli/dist/cli.js check --description \"Rate-limits /api/users to 60 req/min.\"` outputs `Found 1 claim(s)`";
...
function repairPrompt(actualOutput: string): string {
  ...
  return [
    ...
    "",
    "After fixing, re-run:  npx vitest run tests/pinned/v0-1-1-cli-output-contains-node-apps-cli-dist-cli-js-check-description-rate-limits-api-users-to-60-req-min-7llvs2.test.ts",
    "═══════════════════════════════════════════════════════════════",
    "",
  ].join("\n");
}
