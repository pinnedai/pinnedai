...
// ═══════════════════════════════════════════════════════════════
// Original PR claim: "`pinned safety` outputs `Safety Pass`"
// Source PR:         v0-1-1
// Template:          cli-output-contains
// Permanent:         this test fails if the claim is ever regressed.
//
// Retire when no longer applicable:
//   pinned retire v0-1-1-cli-output-contains-pinned-safety-uxvfyu --reason="..."
// ═══════════════════════════════════════════════════════════════

import { describe, it, expect } from "vitest";
import { execFileSync } from "node:child_process";

const COMMAND = "pinned safety";
const ARGV = ["pinned","safety"];
const EXPECTED_TEXT = "Safety Pass";
const ORIGINAL_PR = "v0-1-1";
const ORIGINAL_CLAIM = "`pinned safety` outputs `Safety Pass`";
...
function repairPrompt(actualOutput: string): string {
  ...
  return [
    ...
    "",
    "After fixing, re-run:  npx vitest run tests/pinned/v0-1-1-cli-output-contains-pinned-safety-uxvfyu.test.ts",
    "═══════════════════════════════════════════════════════════════",
    "",
  ].join("\n");
}
