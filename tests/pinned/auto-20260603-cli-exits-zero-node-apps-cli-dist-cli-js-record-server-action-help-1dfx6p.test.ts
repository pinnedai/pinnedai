...
// ═══════════════════════════════════════════════════════════════
// Original PR claim: "`node ./apps/cli/dist/cli.js record-server-action --help` exits 0"
// Source PR:         auto-20260603
// Template:          cli-exits-zero
// Permanent:         this test fails if the claim is ever regressed.
//
// Retire when no longer applicable:
//   pinned retire auto-20260603-cli-exits-zero-node-apps-cli-dist-cli-js-record-server-action-help-1dfx6p --reason="..."
// ═══════════════════════════════════════════════════════════════

import { describe, it, expect } from "vitest";
import { spawnSync } from "node:child_process";

const COMMAND = "node ./apps/cli/dist/cli.js record-server-action --help";
const ARGV = ["node","./apps/cli/dist/cli.js","record-server-action","--help"];
const ORIGINAL_PR = "auto-20260603";
const ORIGINAL_CLAIM = "`node ./apps/cli/dist/cli.js record-server-action --help` exits 0";
const TEST_FILENAME = "auto-20260603-cli-exits-zero-node-apps-cli-dist-cli-js-record-server-action-help-1dfx6p.test.ts";
