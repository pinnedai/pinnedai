...
// ═══════════════════════════════════════════════════════════════
// Original PR claim: "`pinned statusline` outputs `◆ pinned`"
// Source PR:         v0-1-1
// Template:          cli-output-contains
// Permanent:         this test fails if the claim is ever regressed.
//
// Retire when no longer applicable:
//   pinned retire v0-1-1-cli-output-contains-pinned-statusline-dzngcj --reason="..."
// ═══════════════════════════════════════════════════════════════

import { describe, it, expect } from "vitest";
import { execFileSync } from "node:child_process";

const COMMAND = "pinned statusline";
const ARGV = ["pinned","statusline"];
const EXPECTED_TEXT = "◆ pinned";
const ORIGINAL_PR = "v0-1-1";
const ORIGINAL_CLAIM = "`pinned statusline` outputs `◆ pinned`";
...
function repairPrompt(actualOutput: string): string {
  ...
  return [
    ...
    "",
    "After fixing, re-run:  npx vitest run tests/pinned/v0-1-1-cli-output-contains-pinned-statusline-dzngcj.test.ts",
    "═══════════════════════════════════════════════════════════════",
    "",
  ].join("\n");
}
