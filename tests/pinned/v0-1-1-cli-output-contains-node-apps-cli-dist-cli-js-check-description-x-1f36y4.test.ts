...
// Original PR claim: "`node ./apps/cli/dist/cli.js check --description \"x\"` outputs `No claims found`"
// Pinned to fail if this is ever regressed.
//
// Retire when no longer applicable:
//   pinned retire v0-1-1-cli-output-contains-node-apps-cli-dist-cli-js-check-description-x-1f36y4 --reason="..."

import { describe, it, expect } from "vitest";
import { execFileSync } from "node:child_process";

const COMMAND = "node ./apps/cli/dist/cli.js check --description \"x\"";
const ARGV = ["node","./apps/cli/dist/cli.js","check","--description","x"];
const EXPECTED_TEXT = "No claims found";
const ORIGINAL_PR = "v0-1-1";
const ORIGINAL_CLAIM = "`node ./apps/cli/dist/cli.js check --description \"x\"` outputs `No claims found`";
...
function repairPrompt(actualOutput: string): string {
  ...
  return [
    ...
    "",
    "After fixing, re-run:  npx vitest run tests/pinned/v0-1-1-cli-output-contains-node-apps-cli-dist-cli-js-check-description-x-1f36y4.test.ts",
    "═══════════════════════════════════════════════════════════════",
    "",
  ].join("\n");
}
